...
def update_target_list(ship_name, target_listbox, radius=100):
    ...
    if game_instance:
        targetable_enemies = game_instance.get_targetable_enemies(
            ship_name, radius)
...
def create_control_panel(ship_name, position_index):
    ...
    def on_select(evt):
        ...
        if w.curselection():
            ...
            current_ship = game_instance.ships[ship_name]
            # Set selected target for this ship
            current_ship.selected_target = selected_target
    ...
    def fire_command():
        ...
        # Get the selected target for this ship
        selected_target = current_ship.selected_target
    ...
    repair_weapons_button = tk.Button(
        weapons_frame, text="Repair Weapons", command=lambda: post_command(ship_name, "REPAIR weapons"))
    repair_weapons_button.pack()

    def update_weapons_buttons():
        ship = game_instance.ships[ship_name]
        state = "normal" if not ship.deactivated and not ship.disabled_consoles[
            "weapons"] else "disabled"
    ...
    # Science section
    shield_button = tk.Button(science_frame, text="Raise Shields",
                              command=lambda: post_command(ship_name, "TOGGLE_SHIELDS"))
    shield_button.pack()

    repair_shields_button = tk.Button(
        science_frame, text="Repair Shields", command=lambda: post_command(ship_name, "REPAIR shields"))
    repair_shields_button.pack()

    def update_science_buttons():
        ship = game_instance.ships[ship_name]
        state = "normal" if not ship.deactivated and not ship.disabled_consoles[
            "shields"] else "disabled"
    ...
    # Helm section
    stop_button = tk.Button(helm_frame, text="Stop",
                            command=lambda: post_command(ship_name, "STOP"))
    stop_button.pack()
    partial_speed_button = tk.Button(
        helm_frame, text="Partial Speed", command=lambda: post_command(ship_name, "PARTIAL"))
    partial_speed_button.pack()
    full_speed_button = tk.Button(
        helm_frame, text="Full Speed", command=lambda: post_command(ship_name, "FULL"))
    full_speed_button.pack()
    left_button = tk.Button(helm_frame, text="Turn Left",
                            command=lambda: post_command(ship_name, "LEFT"))
    left_button.pack()
    right_button = tk.Button(helm_frame, text="Turn Right",
                             command=lambda: post_command(ship_name, "RIGHT"))
    right_button.pack()

    repair_helm_button = tk.Button(
        helm_frame, text="Repair Helm", command=lambda: post_command(ship_name, "REPAIR helm"))
    ...
    def update_powerup_buttons():
        ship = game_instance.ships[ship_name]
        powerup_count = len(ship.collected_powerups)
        ...
        for powerup_type in ship.collected_powerups:
            if powerup_type not in powerup_buttons:
                ...
                button = tk.Button(
                    engineering_frame, text=f"Activate {powerup_type}", command=make_command())
                button.pack()
    ...
    collect_powerup_button = tk.Button(
        engineering_frame, text="Collect Powerup", command=collect_powerup)
    collect_powerup_button.pack()

    def restore_power():
        post_command(ship_name, "RESTORE_POWER")

    restore_power_button = tk.Button(
        engineering_frame, text="Restore Power", command=restore_power)
